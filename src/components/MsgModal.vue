    ...
    >
      <div class="msgTitle">
        ...
        <div class="msgTitleDiv">
          <TextCustom customFontSize="title" display="inline-block">
            {{ this.msgTitle }}
          </TextCustom>
        </div>
      </div>
      ...
      <div class="msgOkButton">
        <v-btn
          ...
          :color="darkblue1"
          @click="
            this.$root.msgModalEnabled = false;
            this.$root.finishWaitRenderMsgButtonPress(true);
            this.$emit('accept');
          "
        >
          Sim
        ...
        <v-btn
          ...
          @click="
            this.$root.msgModalEnabled = false;
            this.$root.finishWaitRenderMsgButtonPress(false);
            this.$emit('reject');
          "
        >
          Não
        ...
        <v-btn
          v-if="!this.msgAccept && !this.msgReject"
          id="btnmsgok"
          :color="darkblue1"
          @click="
            this.$root.msgModalEnabled = false;
...
export default {
  ...
  created() {
    ...
    this.backgroundWhite = this.white
      .replace(")", ",0.5)")
      .replace("rgb", "rgba");
